...
template<typename V>
struct binary_slash_functor<V, V, std::enable_if_t<is_vector<V>::value>>
  : public elementwise_binary_vector_functor<
      typename V::traits_type,
      binary_slash_functor<typename V::traits_type::element_type,
                           typename V::traits_type::element_type>>
{};

template<typename TRAITS>
auto
operator/=(vector<TRAITS>& u, vector<TRAITS>& v)
{
  u = u / v;
  return u;
}

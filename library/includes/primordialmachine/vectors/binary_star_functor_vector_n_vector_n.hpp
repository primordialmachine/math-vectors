...
struct binary_star_functor<vector<TRAITS>, vector<TRAITS>, void>
  : public elementwise_binary_vector_functor<
      TRAITS,
      binary_star_functor<typename TRAITS::element_type,
                          typename TRAITS::element_type>>
{};

template<typename TRAITS>
auto operator*(vector<TRAITS> const& u, vector<TRAITS> const& v)
{
  return binary_star_functor<vector<TRAITS>, vector<TRAITS>>()(u, v);
}
